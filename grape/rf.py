...
def bp(t_s, flip_angle_rd = pi/2):
    N = t_s.size
    dt = t_s[1] - t_s[0]
    w = 0 # rd/s
    rf = np.ones(N)
    G = flip_angle_rd/(np.sum(rf*dt))
    rf = G*rf
    return rf

def constant_pulse(t_s, amplitude=1000, offset=0):
    N = t_s.size
    dt = t_s[1] - t_s[0]
    w = offset # rd/s
    rf = amplitude * np.ones(N,np.complex64) * np.exp(1j*w*t_s)
    return rf


def sinc(t_s , bandwidth_hz = 2000.0):
    N       = t_s.size
    dt      = t_s[1] - t_s[0]
    rf      = np.zeros(N,np.complex64)
    t_c     = t_s - (t_s[-1] + dt)/2 # centered time interval
    t_c     = np.where(t_c == 0, 1.0e-20, t_c) # to avoid division by 0
    a       = 0.5 # Hann windowing
    tbp     = t_s[-1] * bandwidth_hz
    t0      = t_s[-1] / tbp
    w1_sinc = np.multiply(np.divide((t0*np.sin(pi*t_c/t0)),(pi*t_c)) , ((1-a)+a*np.cos((pi*t_c)/(tbp*t0))))
    w1_sinc[-1] = 0
    return rf

def bir4(t_s, flip_angle_rd = pi/2, beta = 10.0 , kappa = 1.52, dw0 = 80000.0, w1_max=15000):
    rf = np.zeros(t_s.size, dtype=np.complex128)
    dt = t_s[1] - t_s[0]
    Tp = t_s[-1] + dt  # pulse duration

    # Own implementation of BIR-4 pulse design
    # --- Parameters from the paper ---
    delta_phi1 = np.pi + flip_angle_rd / 2       # e.g., for 90° rotation
    delta_phi2 = -delta_phi1            # for symmetry

    # Time vector
    w1 = np.zeros_like(t_s)
    phi = np.zeros_like(t_s)

    # === Core segment 1 functions (Eq. 7, 8) ===
    def w1_seg1(t_seg):
        return w1_max * np.tanh(beta * (1 - 4 * t_seg / Tp))

    def phi_seg1(t_seg):
        cos_term = np.cos(4 * kappa * t_seg / Tp) / np.cos(kappa)
        ln_term = np.log(cos_term)
        return (np.pi / 2) - (dw0 * Tp / (4 * kappa * np.tan(kappa))) * ln_term
    ...
    for i, ti in enumerate(t_s):
        if ti <= 0.25 * Tp:
            ...
            t1 = Tp - ti
            w1[i] = w1_seg1(t1)
            phi[i] = phi_seg1(t1) + delta_phi1 + delta_phi2

    rf = w1 * np.exp(1j * phi)
    return rf

    # # === Plot ===
    # px.line(x=t_s, y=w1, labels={'x': 'Time (s)', 'y': 'RF Amplitude (arb units)'}, title='RF Amplitude vs Time').show()
    # px.line(x=t_s, y=phi, labels={'x': 'Time (s)', 'y': 'Phase (rad)'}, title='RF Phase vs Time').show()


     # from pulpy https://github.com/jonbmartin/pulpy/blob/master/pulpy/rf/adiabatic.py
    ...
    # om = np.concatenate((om1, om2, om3, om4)) # pulsation modulation

    # pm = integrate.cumulative_trapezoid(y=om, dx=dt, initial=0) # phase modulation

    # rf = w1_max * a * np.exp(1j * pm)

    # return rf

def hypsec(t_s, beta = 5.0, mu=672.0, max_w1=15000):
    N = t_s.size
    dt = t_s[1] - t_s[0]
    # from pulpy https://github.com/jonbmartin/pulpy/blob/master/pulpy/rf/adiabatic.py(n=512, beta=800, mu=4.9, dur=0.012):
    ...
    rf = max_w1 * a * np.exp(1j * pm)

    return rf

def sech(t_s, mu = 5, beta = 672, max_w1=12e-6):
    """Design an inversion sech adiabatic pulse."""
    t = t_s - (t_s[-1] + t_s[1]-t_s[0])/2
    A = max_w1 * (1 / np.cosh(beta * t))
    omega_1 = -mu * beta * np.tanh(beta * t)
    phase = cumulative_trapezoid(omega_1, t, initial=0)

    rf = A * np.exp(-1j * phase)
...
def slr(
    ...
    Args:
        ...
        cancel_alpha_phs (bool): For 'ex' pulses, absorb the alpha phase
            ...
            total phase
        flip_angle_rd (float): Targeted flip angle for pulse calibration
...
def fmp(h):
    ...
    padwidths = np.array([np.ceil((lp - ll) / 2), np.floor((lp - ll) / 2)])
...
def root_flip(b, d1, flip, tb, verbose=False):
    ...
    candidates = np.logical_and(
        np.abs(1 - np.abs(r)) > 0.004, np.abs(np.angle(r)) < tb / n * np.pi
    )
